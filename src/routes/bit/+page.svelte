<script lang="ts">
  import BitInteraction, {
    type BitValue,
  } from '$lib/components/bit-interaction.svelte';
  ...
  const journeySteps = $derived(
    buildJourneySteps(
      '02',
      selectedBit === null ? 2 : 3,
      selectedBit === null ? null : '03',
    ),
  );
...
>
  ...
  {#snippet action()}
    <ChapterAdvanceKey
      ...
      label="Go quantum"
      icon="atom"
      ariaLabel="Go quantum to the qubit chapter"
      beforeNavigate={prepareQubitPage}
    />
  {/snippet}
</JourneyShell>
